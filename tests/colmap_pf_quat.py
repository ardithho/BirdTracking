...
from utils.filter import ParticleFilterQuat
        ...
        if bird is not None:
            ...
            if head_pts.shape[0] >= 4:
                ...
                if pnp is not None:
                    ...
                    R = R.T
                    # particle filter
                    obs = np.array([*Rotation.from_matrix(R).as_quat(), *t])
                    pf.transition()
                    pf.observe(obs, continuous)
                    pf.resample()
                    estimate = pf.estimate()
                    R = Rotation.from_quat(estimate[:4]).as_matrix()
                    t = estimate[4:]
