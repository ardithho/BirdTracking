...
def try_export(inner_func):
    ...
    def outer_func(*args, **kwargs):
        ...
        try:
            ...
            LOGGER.info(f'{prefix} export success ✅ {dt.tvec:.1f}s, saved as {f} ({file_size(f):.1f} MB)')
            return f, model
        except Exception as e:
            LOGGER.info(f'{prefix} export failure ❌ {dt.tvec:.1f}s: {e}')
            return None, None
...
def run(
    ...
    if any(f):
        ...
        LOGGER.info(f'\nExport complete ({time.time() - t:.1f}s)'
                    ...
                    f"\nDetect:          python {dir / ('predict.py' if det else 'predict.py')} --weights {f[-1]} {h}"
