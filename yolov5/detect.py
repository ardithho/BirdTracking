...
Usage - formats:
    $ python predict.py --weights yolov5s.pt                 # PyTorch
...
from utils.sorter import filter_feat, to_txt
from utils.plot import plot_feat
...
def run(
        ...
        project=PROJECT_ROOT / 'runs/detect',  # save results to project/name
    ...
    for path, im, im0s, vid_cap, s in dataset:
        with dt[0]:
            im = torch.from_numpy(im).to(model_head.device)
            im = im.half() if model_head.fp16 else im.float()  # uint8 to fp16/32
        ...
        with dt[1]:
            ...
            pred = model_head(im, augment=augment, visualize=visualize)
...
def parse_opt():
    ...
    parser.add_argument('--project', default=PROJECT_ROOT / 'runs/detect', help='save results to project/name')
